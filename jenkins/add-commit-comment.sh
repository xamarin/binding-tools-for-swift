...
if ! curl -f -v -H "Authorization: token $TOKEN" -H "User-Agent: command line tool" -d "@$JSONFILE" "https://api.github.com/repos/$GIT_ORG/$GIT_REPO/commits/$HASH/comments" > "$LOGFILE" 2>&1; then
	...
	echo "Successfully added commit message to https://github.com/$GIT_ORG/$GIT_REPO/commit/$HASH"
fi
